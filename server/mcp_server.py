...
_SOURCE_TOKEN_RE = re.compile(
    ...
    r'amazon|netflix|hulu|coursera|udemy|khan|mozilla|mdn|w3schools|datastudios'
    r')$',
    re.IGNORECASE,
)
        ...
        () => {
            ...
            document.querySelectorAll('a[href]').forEach(a => {
                ...
                if (parent || /^\\d+$/.test(txt) || a.closest('sup, [class*="cite"], [class*="Source"]')) {
                    seen.add(href);
                    results.push({ index: results.length + 1, title: document.title || txt || href, url: href });
                }
            });
            if (results.length === 0) {
                document.querySelectorAll('a[href]').forEach(a => {
                    ...
                    if (href.startsWith('http') && !href.includes('perplexity.ai')) {
                        ...
                        if (txt && txt.length < 60) {
                            seen.add(href);
                            results.push({ index: results.length + 1, title: txt, url: href });
                        }
                    }
                });
            }
...
def _clean_answer_text(text: str) -> str:
    ...
    for line in lines:
        stripped = line.strip()
        if stripped and _SOURCE_TOKEN_RE.match(stripped.rstrip('.,;:+0123456789 ')):
            continue
        cleaned.append(line)
    result = re.sub(r'\n{3,}', '\n\n', '\n'.join(cleaned))
    return result.strip()
